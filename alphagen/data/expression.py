...
class Cov(PairRollingOperator):
    def _apply(self, lhs: Tensor, rhs: Tensor) -> Tensor:
        ...
        # ChatGPT
        chunk_size = 10  # 根据显存大小调整分块数
        return self.apply_in_chunks(lhs, rhs, chunk_size)

    def apply_in_chunks(self, lhs: Tensor, rhs: Tensor, chunk_size: int) -> Tensor:
        n = lhs.shape[-1]
        results = []
        for lhs_chunk, rhs_chunk in zip(torch.chunk(lhs, chunk_size, dim=0), torch.chunk(rhs, chunk_size, dim=0)):
            clhs = lhs_chunk - lhs_chunk.mean(dim=-1, keepdim=True)
            crhs = rhs_chunk - rhs_chunk.mean(dim=-1, keepdim=True)
            result = (clhs * crhs).sum(dim=-1) / (n - 1)
